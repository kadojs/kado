...
const StreamSlicer = require('../lib/StreamSlicer')
...
runner.suite('StreamSlicer', (it, suiteStreamSlicer) => {
  it('should construct', () => {
    Assert.isType('StreamSlicer', new StreamSlicer({ boundary: '-----' }))
  })
  it('should end and finish', () => {
    ...
    const test2 = () => {
      streamSlicer2 = new StreamSlicer({ boundary: boundary })
    ...
    const streamSlicer = new StreamSlicer({ boundary: boundary })
  ...
  suiteStreamSlicer.suite('HeaderParser', (it) => {
    ...
    tests.forEach(v => {
      it(`${v.what}`, () => {
        const parser = new StreamSlicer.HeaderParser()
        let fired = false
  ...
  suiteStreamSlicer.suite('multipart', (it, suiteMultipart) => {
    ...
    tests.forEach(v => {
      it(`${v.what}`, () => {
        ...
        const streamSlicer = new StreamSlicer(v.opts)
        let error
        let partErrors = 0
        let finishes = 0
