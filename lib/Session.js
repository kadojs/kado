...
class SessionStore {
  constructor (options = {}) {
    ...
    this.pruneInterval = setInterval(() => {
      const count = this.prune()
      if (count) console.log(`Pruned ${count} Session entries.`)
    }, this.pruneFrequency)
  }
...
class SessionStorage {
  ...
  constructor (options = {}) {
    ...
    this.net = net
    this.port = options.port || 3001
    this.saveFile = options.saveFile || fs.path.join(__dirname, '.session.json')
...
class Session {
  ...
  static generateId (req, trustProxy = false) {
    const sha1 = crypto.createHash('sha1')
    let ip = req.socket.localAddress
    if (req.headers['x-forwarded-for'] && trustProxy) {
      ip = req.headers['x-forwarded-for']
    }
    const idString = +(new Date()) + ip + req.headers['user-agent']
  ...
  static request (req, res, options) {
    req.session = new Session(
      req.cookie.session || Session.generateId(req, options.trustProxy),
      options.store
    )
    if (!req.cookie.session) {
      res.setHeader('Set-Cookie', 'session=' + req.session.sid)
    }
    res.once('close', () => { return req.session.save() })
    return req.session.restore()
  }
