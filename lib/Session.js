...
class SessionStoreSQL extends SessionStore {
  ...
  save (sid, data) {
    ...
    return this.db.getEngine().execute(query.toString(), query.toArray())
      .then((result) => {
        if (result[0] && result[0][0] && result[0][0].sid === sid) {
          const query = this.model.update(['data', 'updatedAt'])
  ...
  restore (sid) {
    ...
    return this.db.getEngine().execute(query.toString(), query.toArray())
      .then((result) => {
        ...
        if (!result[0][0].data || !result[0][0].data.match(/^{/)) return {}
        return JSON.parse(result[0][0].data)
      })
  }
