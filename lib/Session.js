  ...
  async save (
    ...
    if (row && row.sid === sid) {
      ...
      try {
        ...
        if (err.message.match(/Duplicate entry/) && attempt < 5) {
          return this.save(sid, ip, agent, uid, data, ++attempt)
        } else {
          throw err
        }
      }
...
class Session {
  ...
  set (key, value, options = {}) {
    ...
    this.data[key] = value
    this.written = true
    if (options.save === false) return this
    return this.save()
  }
  ...
  async save () {
    ...
    return this.store.save(
      this.sid, this.ip, this.agent, this.uid, this.data
    )
  }
}
