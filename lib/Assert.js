...
class Assert extends Validate {
  ...
  static ensure (type, val) {
    const valType = Assert.getType(val).toLowerCase()
    type = type.toLowerCase().trim()
    try {
      if (type === 'array' || type === 'arr') {
        if (valType !== 'array') val = [val]
        return val
      }
      if (type === 'object' || type === 'obj') {
        if (valType !== 'object' || Array.isArray(val)) val = { a: val }
        return val
      }
      if (type === 'number' || type === 'num' || type === 'int') {
        if (valType !== 'number') val = Number(val)
        return val
      }
      if (type === 'string' || type === 'str') {
        if (valType !== 'string') val = String(val)
        return val
      }
      // no change (unsupported coercion)
      return val
    } catch (err) {
      throw new Assert.Error('Failed to ensure type(' + type + ') from ' +
        valType + ' on ' + val + ': ' + err.message)
    }
  }
