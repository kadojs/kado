...
class ViewMustache extends ViewEngine {
  constructor (viewFolder, options = {}) {
    super()
    try { require.resolve('mustache') } catch (e) {
      if (e instanceof Error) {
        console.log('Mustache not found, try: npm install mustache')
      }
    }
    ...
    this.partialCache = options.partialCache !== undefined
      ? options.partialCache : false
    this.template = {}
    this.templateCache = options.templateCache !== undefined
      ? options.templateCache : false
  ...
  render (req, res, templateName, params, options = {}) {
    ...
    this.loadTemplate(templateName, tags)
      .then((result) => {
        ...
        res.end(Mustache.render(template, params, partials, tags))
      })
      .catch((err) => {
        if (err instanceof Error) {
          ...
          console.error('Unknown rendering error', err)
          res.statusCode = 500
          res.end('Unknown rendering error')
        }
      })
  }
}
