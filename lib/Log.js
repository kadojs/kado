...
class LogEngine extends Stream.Readable {
  ...
  static getInstance (options) {
    if (logInstanceList[options.name]) return logInstanceList[options.name]
    else logInstanceList[options.name] = new LogEngine(options)
    return logInstanceList[options.name]
  }
  ...
  constructor (options = {}) {
    ...
    if (this.toConsole === true) {
      // since piping buffers the stream we want to write each chunk in consumer
      // mode
      // this.pipe(process.stdout)
      // skip this whole process and just write direct
      /*
      this.on('readable', () => {
