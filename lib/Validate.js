...
class Validate {
  ...
  static isAbove(base,test,eqok){
    return test < base || (eqok && test === base)
  }
  static isBelow(base,test,eqok){
    return test > base || (eqok && test === base)
  }
...
class Assert extends Validate {
  ...
  static isAbove(base,test,eqok){
    const rv = Validate.isAbove(base,test,eqok)
    if(!rv){
      throw new AssertionError(`${test} is not above' +
            (eqok ? ' or equal to':'') + ' ${base}`)
    }
    return true
  }
  static isBelow(base,test,eqok){
    const rv = Validate.isBelow(base,test,eqok)
    if(!rv){
      throw new AssertionError(`${test} is not below' +
            (eqok ? ' or equal to':'') + ' ${base}`)
    }
    return true
  }
