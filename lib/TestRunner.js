...
class TestReporter {
  ...
  testComplete(test,noun,extra){
    ...
    console.log(`${wsp}${flag} - ${test.testName} ${noun}${duration}${extra}`)
  }
  suiteComplete(suite){
    let flag = TestReporter.color('o',TestColor.ok())
    ...
    let wsp = ' '
    for(let i = 0; i < suite.level; i++){ wsp += ' ' }
    console.log(`${wsp}${flag} - ${suite.name} ` +
      ...
      .then(()=>{
        ...
        return new Promise((resolve,reject)=> {
          test.timeout = (timeoutMs) => {
            setTimeout(
              () => { reject(`Exceeded timeout of ${timeoutMs}ms`) },
              timeoutMs
            )
          }
          const rv = test.call(test)
          if(rv instanceof Promise) rv.then(resolve).catch(reject)
          else if(false === rv) reject('Returned false')
          else resolve()
        })
      })
      .then((result)=> {
  ...
  execute(options){
    ...
    return Promise.resolve().then(()=> {
      ...
      .then(()=>{
        ...
        TestReporter.failedTests(this.failed)
        return testsFailing
      })
  }
}
