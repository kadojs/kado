...
class MustacheUtil {
  ...
  static escapeHtml (string) {
    if (string instanceof Date) string = string.toDateString()
...
class MustacheWriter {
  ...
  render (template, view, partials, tags) {
    ...
    const context = (view instanceof MustacheContext) ? view
  ...
  renderPartial (token, context, partials, tags) {
    if (!partials) return
    const value = Validate.isType('function', partials) ? partials(token[1])
      : partials[token[1]]
    if (value != null) {
