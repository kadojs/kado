...
class CommandLine {
  ...
  runProgram(module,command,argv){
    const that = this
    if(!this.commandProgram[module] || !this.commandProgram[module][command]){
      throw new Error(`Missing program ${module} ${command}`)
    }
    let promise = null
    this.commandProgram[module][command].action((opts)=>{
      promise = P.try(()=>{
    ...
    if(!promise){
      throw new Error(`Failed to invoke command: ${module} ${command}`)
    }
    return promise
  }
