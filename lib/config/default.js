...
module.exports = {
  ...
  log: {
    ...
    winston: {
      enabled: true,
      default: true,
      root: path.resolve(path.join(__dirname,'logger','winston.js')),
      transports: {
        console: {
          enabled: true,
          level: null
  ...
  db: {
    ...
    sequelize: {
      enabled: false,
      load: false,
      root: path.resolve(path.join(libRoot,'db','sequelize.js')),
      skipConnect: false,
      name: 'kado',
      host: '127.0.0.1',
      port: 3306,
  ...
  module: {
    kado: {
      enabled: true,
      root: path.resolve(path.join(root,'kado_modules','kado','kado.js'))
    }
  }
}
