...
class CommandServer {
  ...
  checkParser () {
    Assert.isOk(this.parser, 'No parser defined for CommandServer')
  }
  ...
  command (module, name, action, options, description) {
    ...
    if (!options) options = {}
    options.name = name
    options.description = description || name
  ...
  generateHelp (module) {
    let help = `Usage of ${'app' + (module ? ' ' + module : '')}` +
      ': [command]\nCommands:\n'
    const opts = []
    opts.unshift({ definition: '-V, --version', description: 'output version' })
    opts.unshift({ definition: '-h, --help', description: 'output help' })
    ...
    if (module && module !== '--help') {
      commandList = this.commands[module]
    }
    for (const cmd in commandList) {
      let newLine = `  ${cmd}`
      while (newLine.length < 25) { newLine = newLine + ' ' }
      if (commandList[cmd] && commandList[cmd].options) {
        newLine = newLine + (commandList[cmd].options.description || cmd)
      } else {
        newLine = newLine + cmd
      }
      help = help + `${newLine}\n`
  ...
  printHelp (module) {
    console.log(this.generateHelp(module))
  }

  run (command) {
    // so this is where we would basically parse and run a command artificially
    // but since we don't register all the commands yet like we should this
    // won't quite work right, now this is becoming some sweet api, and I don't
