...
class ChunkStream extends Stream.Readable { _read (n) { super._read(n) } }

class Mutlipart extends Stream.Writable {
  static getInstance (opts) { return new Mutlipart(opts) }
  ...
  constructor (opts) {
    ...
    this._nparts = 0
    this._pause = false
    this.boundary = Mutlipart.getBoundary(opts.headers['content-type'])
  ...
  parserPartHeader (header) {
    ...
    if (conType === 'application/octet-stream' || filename !== undefined) {
      ...
      ++this.nends
      const file = new ChunkStream(this.fileOpts)
      this.curFile = file
      file.on('end', () => {
        --this.nends
        this._pause = false
  ...
  write (chunk, cb) {
    ...
    const paused = this._pause
    if (parseWrite && !paused) return cb()
    this._needDrain = !parseWrite
    this._cb = cb
  }
}

module.exports = Mutlipart
