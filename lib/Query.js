...
class Query {
  static addBase (table, selection, construct, tableConstruct) {
    if (!construct) construct = 'SELECT'
    if (!tableConstruct) tableConstruct = 'FROM'
...
class QuerySQL extends Query {
  ...
  on (item, comparison, value, joiner) {
    this.qon = Query.addComparison(
      this.qon, 'ON', item, comparison, value, joiner
    )
    return this
  }
  ...
  where (item, comparison, value, joiner) {
    this.qwhere = Query.addComparison(
      this.qwhere, 'WHERE', item, comparison, value, joiner
    )
    return this
  }
