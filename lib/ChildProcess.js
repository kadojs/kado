...
class ChildProcess extends EventEmitter {
  ...
  respawnChild () {
    if (this.respawn === true && this.started === true) {
  ...
  addChild (proc) {
    ...
    proc.on('close', () => { this.respawnChild() })
    ...
    return Promise.resolve()
      .then(() => {
        if (this.started === true) {
          throw new Error(`Process ${this.path} already started: ${this.pid}`)
        }
        if (this.path === false) {
          throw new Error(`${this.path} must be an executable program`)
        }
    ...
    return Promise.resolve()
      .then(() => {
        if (this.started === false) {
          throw new Error(`Child process ${this.path} is not running`)
        }
        // prevent re spawns
        this.started = false
        const promises = []
  ...
  send (message, sendHandle, options) {
    ...
    for (const i in this.child) {
      if (!Object.prototype.hasOwnProperty.call(this.child, i)) continue
      rv[i] = this.child[i].send(message, sendHandle, options, (e) => {
