...
class Kado {
  ...
  registerMethodHandlers(){
    ...
    this.getCurrentNodeMethods().forEach((method)=>{
      that[method] = function(){
        ...
        that.route.p(args[0]) //register route with kado
  ...
  extendExpress(){
    ...
    this.express.locals._uri = this.route
    ...
    this.express.use((req,res,next)=>{
      ...
      res.render = (tpl,options,cb) => {
        ...
        res.locals._css = that.asset.all('text/css')
        res.locals._script = that.asset.all('text/javascript')
        ...
        res._r(tpl,options,(err,output)=>{
          ...
          if(err){
            that.log.warn('Failed to render: ' + err.message)
            res.status(500)
          }
          res.send(output)
        })
