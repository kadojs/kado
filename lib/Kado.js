...
class Kado {
  ...
  addModule(moduleRoot){
    ...
    if('function' === typeof mod.config) mod.config(this.config)
    if('function' === typeof mod.db){
      mod.db(this,this.db,this.db.sequelize.instance)
    }
  ...
  loadDatabase(){
    ...
    Object.keys(this.config.db).forEach((databaseName)=>{
      ...
      if(databaseConfig.load === true && databaseConfig.enabled === true){
        ...
        that.db[databaseName] = ThisDatabase.get()
        that.database.addDatabase(
          databaseName,
          new ThisDatabase(databaseConfig)
        )
      }
    })
  }
  ...
  setDevMode(){
    ...
    let prodEnvFile = path.resolve('./.env')
    let prodEnvFileUser = path.resolve(
      process.env.KADO_USER_ROOT + '/.env'
    )
    let devEnvFile = path.resolve('./.env_dev')
    let devEnvFileUser = path.resolve(
  ...
  render(tpl,options,cb){
    ...
    if(this.asset){
      this.locals._css = this.asset.all('text/css')
      this.locals._script = this.asset.all('text/javascript')
    }
  ...
  start(){
    ...
    return new P((resolve,reject)=>{
      ...
      if(!that.server){
        that.server = that.express.listen.apply(that.express,args)
      } else {
        that.server.listen.apply(that.express,args)
      }
    })
      .then((result)=>{
        startResult = result
