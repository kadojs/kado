...
class Kado {
  ...
  helper(name){
    ...
    let userHelper = path.resolve(this.KADO_USER_ROOT +
      '/lib/' + name + '.js')
    let kadoHelper = path.resolve(this.KADO_ROOT +
      '/lib/' + name + '.js')
  ...
  extendExpress(){
    ...
    this.express.use((req,res,next) => {
      ...
      let packList = that.lang.all()
      let packs = []
      Object.keys(packList).forEach((key)=>{
        packs.push(packList[key])
      })
    ...
    this.express.use((req,res,next) => {
      ...
      res.locals._u = that.router.allForTemplate()
      next()
    })
    //load uri overrides
