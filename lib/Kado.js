...
/**
 * Get current HTTP Methods for Node.JS
 * @returns {string[]}
 */
...
class Kado {
  constructor(config,configLocal){
    this.CommandLine = require('./CommandLine')
  ...
  routeCli(args){
    ...
    let mod = require(path.resolve(path.join(module._kado.root,'kado.js')))
    P.try(()=>{
      return mod.cli(this,args)
    })
      .then((result)=>{
