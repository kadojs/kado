...
const LogHelper = require('./logger/winston')
...
class Kado {
  ...
  loadLoggers(){
    ...
    Object.keys(this.config.log).forEach((logKey) => {
      ...
      const logger = new LogHelper(that.config.name,that.config.log[logKey])
    ...
    this.log = that.logger.getLogger()
  }
  loadLanguagePacks(){
    const that = this
  ...
  test(filter){
    ...
    let env = process.env
    env.KADO_TEST = 'kado'
    let testRunner = path.resolve(path.join(
  ...
  routeCli(args){
    ...
    require(path.resolve(path.join(module._kado.root,'kado.js'))).cli(this,args)
  }
  start(){
    const that = this
    return P.try(()=>{
