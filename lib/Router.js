...
class Router {
  ...
  static renderJSON (res, options = {}) {
    return (object = {}) => {
      let payload = JSON.stringify(object)
      // adapted from the work done on express 5
      //   credit Doug Wilson and Greg Guthe
      if (options.escape !== false) {
        payload = payload.replace(/[<>&]/g, function (c) {
      ...
      res.setHeader('Content-Length', Buffer.byteLength(payload, encoding))
