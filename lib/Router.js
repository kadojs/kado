...
class Router {
  ...
  static standardPreparation (app) {
    ...
    return function standardPreparation (req, res) {
      ...
      req.ip = Route.getRemoteIP(app, req)
      res.json = Route.renderJSON(res)
      req.query = new URL(`http://localhost${req.url}`).searchParams
      res.redirect = Route.redirectRequest(res)
      res.render = Route.render(app, req, res)
      Route.setPoweredBy(res)
