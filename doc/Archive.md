...
> Stability: 2 Stable
```js
const Archive = require('kado/lib/Archive')
```
The `Archive` library provides parsing and extraction of various archive file
formats.

## Class: Archive

This parent class groups the subclasses which handle specific formats.

## Class: Archive.Unzip
This is the reference to the `Unzip` class that can handle `.zip` files,
based on `yauzl` with heavy modification, but functionally similar.

[the original `yauzl` may be found here](https://github.com/thejoshwolfe/yauzl)
...
```js
zipFile.once('end', () => { zipFile.close() })
```
...
`decodeStrings` is the default and causes `Archive.Unzip` to decode strings with `CP437` or `UTF-8` as required by the spec.
...
```js
const errorMessage = Archive.Unzip.validateFileName(fileName)
...
`Archive.Unzip` notices this false information and emits an error from the `readStream`
...
then the file name in this field is stored in the `fileName` property,
...
which makes the library either dishonest or non-conformant (usually the latter).
...
If a zip file contains file entries encrypted with "Strong Encryption", `Archive.Unzip` emits an error.
